...
=======
>>>>>>> 83204c663cde4b2516accf9ba3e3921f5f38b4ae
...
foward_elimation = function(x, y){
  min_value = tidy(lm(y ~ x[[1]], data = x))$p.value[2]
  min_index = 1
  for(i in 2:length(x)-1){
    if(tidy(lm(y ~ x[[i]], data = x))$p.value[2]>min_value){
      min_value = min_value
      min_index = min_index
    }else{
      min_value = tidy(lm(y ~ x[[i]], data = x))$p.value[2]
      min_index = i
    }
...
tidy(lm(y ~ x[[i]], data = x))$p.value[2] > tidy(lm(y ~ x[[i-1]], data = x))$p.value[2]

tidy(lm(log_sal94 ~ lawsuit_log_94[[1]], data = lawsuit_log_94))


names(lawsuit_log_94)[1]
