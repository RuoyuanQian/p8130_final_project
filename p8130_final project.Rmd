...
summ%>% 
  knitr::kable(caption = "Table 1  Descriptive table for all variables of interest",
               format = "html") 
```
...
According to relevant articles, "rank" and "exper" are the interaction terms for gender. 
...
lawsuit_w_exp %>% ggplot(aes(x = exper, y = log_sal,color = exper_ind)) + 
  geom_point() + 
  geom_smooth(method = 'lm', formula = y ~ x)+
  geom_line(aes(x = 10.5),color = "blue")+
    labs(y = "log(average salary)",
         x = "experience")
```
...
In Assistant and <=10 level, the gender is significant predictor.
...
title("Assistant",line=-1)
plot(rank.asso, which = 1)
title("Associate",line=-1)
plot(rank.full, which = 1)
title("Full professor",line=-1)
plot(exper.tenplus, which = 1)
title("experience>10",line=-1)
...
lawsuit_w_exp %>% filter(rank == "Associate") %>% dplyr::select(-rank, - exper_ind) %>% ggplot(aes(x=exper, y = log_sal)) + geom_point() + labs(title = "Associate",y = "log(average salary)") + theme(plot.title = element_text(hjust = 0.5))+
lawsuit_w_exp %>% filter(rank == "Assistant") %>% dplyr::select(-rank, - exper_ind) %>% ggplot(aes(x=exper, y = log_sal)) + geom_point()  + labs(title = "Assistant",y = "log(average salary)") + theme(plot.title = element_text(hjust = 0.5))+
lawsuit_w_exp %>% filter(rank == "Full professor") %>% dplyr::select(-rank, - exper_ind) %>% ggplot(aes(x=exper, y = log_sal)) + geom_point() + plot_layout(ncol = 2, nrow = 2) + labs(title = "Full Professor",y = "log(average salary)" ) + theme(plot.title = element_text(hjust = 0.5))
```
...
```{r}
par(mfrow=c(2,3))
plot(rank.assi_no, which = 1)
title("Assistant",line=-1)
plot(rank.asso_no, which = 1)
title("Associate",line=-1)
plot(rank.full_no, which = 1)
title("Full professor",line=-1)
plot(exper.tenplus, which = 1)
title("experience>10",line=-1)
plot(exper.tenminus, which = 1)
...
```{r}
par(mfrow=c(2,3))
plot(rank.assi_no, which = 2, xlab = "Assistant")
plot(rank.asso_no, which = 2, xlab = "Association")
plot(rank.full_no, which = 2, xlab = "Full professor")
plot(exper.tenplus, which = 2, xlab = "More than 10 years")
plot(exper.tenminus, which = 2, xlab = "Less than 10 years")
```
...
tab_model(rank.asso_no,rank.assi_no,rank.full_no)
tab_model(exper.tenplus,exper.tenminus)
